...
<div class="max-w-3xl mx-auto">
    <div class="bg-white shadow-xl rounded-2xl p-6 sm:p-8">
        <form action="{{ route('admin.blog.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-6" onsubmit="return validateForm(event)">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="3"
                          ...
                          oninput="checkExcerptRealtime(this)">{{ old('excerpt', $post->excerpt) }}</textarea>
            ...
            <div>
                ...
                <div class="document-editor rounded-lg overflow-hidden border border-gray-300">
                    ...
                    <div class="document-editor__editable-container bg-gray-100 p-4 flex justify-center overflow-y-auto max-h-[600px]">
                        <div class="document-editor__editable bg-white shadow-sm border border-gray-200 w-[21cm] min-h-[29.7cm] p-4 sm:p-8" id="editor">{!! old('isi', $post->isi) !!}</div>
                    </div>
                </div>
                <textarea name="isi" id="isi_hidden" class="hidden" required>{{ old('isi', $post->isi) }}</textarea>
                @error('isi')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
...
// Cek nilai existing saat halaman edit dibuka
