    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <span id="online-count"
              class="text-xs text-gray-500 hidden sm:inline">• Memuat pengguna online…</span>
      </div>
  ...
  <div class="bg-white rounded-2xl shadow">
    ...
    <div id="tab-history" class="p-4 sm:p-6 hidden">
      ...
      <div class="mt-3 flex flex-wrap gap-2">
        <button data-chip="today"   class="chip-quick px-3 py-1.5 rounded-full text-xs border hover:bg-gray-50">Hari ini</button>
        <button data-chip="week"    class="chip-quick px-3 py-1.5 rounded-full text-xs border hover:bg-gray-50">Minggu ini</button>
        <button data-chip="month"   class="chip-quick px-3 py-1.5 rounded-full text-xs border hover:bg-gray-50">Bulan ini</button>
        <button data-chip="deleted" class="chip-quick px-3 py-1.5 rounded-full text-xs border hover:bg-gray-50">Hanya yang dihapus</button>
        <button data-chip="poll"    class="chip-quick px-3 py-1.5 rounded-full text-xs border hover:bg-gray-50">Hanya poll</button>
        <button data-chip="attach"  class="chip-quick px-3 py-1.5 rounded-full text-xs border hover:bg-gray-50">Hanya lampiran</button>
        {{-- NEW: voice --}}
        <button data-chip="voice"   class="chip-quick px-3 py-1.5 rounded-full text-xs border hover:bg-gray-50">Hanya voice</button>
      </div>
      ...
      <div id="history-wrapper" class="mt-5 space-y-4">
        ...
        <div id="history-loadmore-wrap" class="text-center hidden">
          <button id="history-loadmore" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm">Muat lebih banyak</button>
        </div>
      </div>
    </div>
...
<div id="tab-settings" class="p-4 sm:p-6 hidden">
  ...
  @php
    ...
    $pdVal = ''; $pdUnit = 'minutes';
    $m = (int) ($settings->default_poll_duration_minutes ?? 0);
    if ($m > 0) {
      if ($m % (60*24) === 0) { $pdUnit = 'days';  $pdVal = (int) ($m / (60*24)); }
      elseif ($m % 60 === 0)  { $pdUnit = 'hours'; $pdVal = (int) ($m / 60); }
      else { $pdUnit = 'minutes'; $pdVal = $m; }
    }
  @endphp
...
<input id="allow_attachments" type="checkbox" name="allow_attachments" value="1"
       class="sr-only peer"
       @checked(old('allow_attachments', $settings->allow_attachments ?? false)))>

          {{-- track + knob (animasi) --}}
          <span aria-hidden="true"
                class="relative inline-block h-6 w-11 rounded-full bg-gray-300 transition-colors
...
<input id="allow_polls" type="checkbox" name="allow_polls" value="1"
       class="sr-only peer"
...
<input id="allow_voice" type="checkbox" name="allow_voice" value="1"
       class="sr-only peer"
    ...
    <div class="rounded-2xl border bg-white p-4 sm:p-5 space-y-5">
      ...
      <div>
        ...
        <div class="mt-1 relative">
          ...
          <span class="absolute right-2 top-1/2 -translate-y-1/2 text-xs bg-gray-100 border px-2 py-1 rounded">MB</span>
        </div>
      ...
      <div>
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2">
          <input type="number" min="1" step="1" name="poll_duration_value"
                 ...
                 class="border rounded-xl px-3 py-2 w-28 focus:ring-2 focus:ring-blue-500"
                 placeholder="Angka">

          @php $unitOld = old('poll_duration_unit', $pdUnit); @endphp
          <div class="inline-flex rounded-xl border p-0.5">
            <label class="cursor-pointer">
              <input type="radio" name="poll_duration_unit" value="minutes" class="sr-only peer" {{ $unitOld==='minutes'?'checked':'' }}>
              <span class="px-3 py-1.5 text-sm rounded-lg block peer-checked:bg-blue-600 peer-checked:text-white">menit</span>
            </label>
            <label class="cursor-pointer">
              <input type="radio" name="poll_duration_unit" value="hours" class="sr-only peer" {{ $unitOld==='hours'?'checked':'' }}>
              <span class="px-3 py-1.5 text-sm rounded-lg block peer-checked:bg-blue-600 peer-checked:text-white">jam</span>
            </label>
            <label class="cursor-pointer">
              <input type="radio" name="poll_duration_unit" value="days" class="sr-only peer" {{ $unitOld==='days'?'checked':'' }}>
              <span class="px-3 py-1.5 text-sm rounded-lg block peer-checked:bg-blue-600 peer-checked:text-white">hari</span>
            </label>
          </div>
        </div>
    ...
    <div id="tab-export" class="p-4 sm:p-6 hidden space-y-6">
      ...
      <div class="flex items-center gap-2 mt-3">
        <button id="btn-download-json"
                class="px-4 py-2 bg-gray-800 text-white rounded-lg text-sm hover:bg-gray-900 disabled:opacity-40"
                disabled>
          Download JSON
        </button>
        ...
        <aside class="lg:col-span-1 space-y-3">
          <div class="rounded-xl border bg-white p-4">
            ...
            <div class="space-y-2">
              ...
              <button id="rep-apply" class="w-full px-3 py-2 rounded-lg bg-blue-600 text-white text-sm">Terapkan</button>
            </div>
          </div>
        </aside>
