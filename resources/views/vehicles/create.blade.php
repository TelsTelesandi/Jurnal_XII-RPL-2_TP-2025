...
<div class="max-w-3xl mx-auto px-4">
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-sky-600 via-cyan-600 to-teal-600 shadow-2xl mb-8">
        ...
        <div class="relative px-8 py-10 text-center">
            ...
            <h1 class="text-3xl font-extrabold text-white mb-1">Tambah Kendaraan</h1>
            <p class="text-cyan-100">Masukkan detail kendaraan untuk digunakan driver</p>
        </div>
    </div>
    ...
    <form method="post" action="{{ route('vehicles.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <select name="active" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                <option value="1" @selected(old('active', '1')=='1')>Ya</option>
                <option value="0" @selected(old('active')=='0')>Tidak</option>
            </select>
        </div>
